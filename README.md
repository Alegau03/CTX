...
tar -xzf ctx-0.2.1-aarch64-apple-darwin.tar.gz
mkdir -p "$HOME/.local/bin"
install -m 0755 ctx-0.2.1-aarch64-apple-darwin/ctx "$HOME/.local/bin/ctx"
...
```bash
sudo install -m 0755 ctx-0.2.1-aarch64-apple-darwin/ctx /usr/local/bin/ctx
```
