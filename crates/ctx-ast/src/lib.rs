...
fn extract_c_symbols(code: &str, tree: &Tree, file_path: &str) -> Vec<RawSymbol> {
    ...
    while let Some(node) = stack.pop() {
        match node.kind() {
            ...
            // Function prototypes / declarations — `int foo(int);`
            "declaration" => {
                if let Some(sym) = extract_function_prototype(code, file_path, node) {
                    symbols.push(sym);
                }
...
/// Extract a function prototype from a `declaration` or `field_declaration` node.
/// Matches things like `int foo(int);` or `virtual void bar() const = 0;` —
/// declarations whose declarator is (or wraps) a `function_declarator`.
fn extract_function_prototype(code: &str, file_path: &str, node: Node<'_>) -> Option<RawSymbol> {
    let declarator = node.child_by_field_name("declarator")?;
    if !declarator_is_function(declarator) {
        return None;
    }
    let name = find_function_name(code, declarator)?;
    let signature = first_line(node_text(code, node));
    let kind = if name.starts_with("test_") || name.starts_with("Test") {
        SymbolKind::Test
    } else {
    ...
    Some(raw_symbol(file_path, &name, kind, &signature, node))
}

fn declarator_is_function(node: Node<'_>) -> bool {
    match node.kind() {
    ...
    while let Some(node) = stack.pop() {
        match node.kind() {
            ...
            "template_declaration" => {
                // Extract the inner class or function name, then walk into the body
                // so inline methods / nested types still get indexed.
                let mut cursor = node.walk();
                for child in node.children(&mut cursor) {
                    match child.kind() {
                        "class_specifier" | "struct_specifier" => {
                            ...
                            for grandchild in child.children(&mut bc) {
                                if grandchild.kind() == "field_declaration_list" {
            ...
            // Function prototypes — `int foo(int);` at file or namespace scope
            // and class member function declarations.
