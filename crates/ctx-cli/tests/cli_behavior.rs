...
#[test]
fn release_version_references_target_v0_2_2() {
    let root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    ...
    assert!(root_manifest.contains("version = \"0.2.2\""));
    assert!(cli_manifest.contains("version.workspace = true"));
    assert!(npm_manifest.contains("\"version\": \"0.2.2\""));
    assert!(formula.contains("v0.2.2"));
    assert!(build_script.contains("VERSION=\"${VERSION:-0.2.2}\""));
    assert!(readme.contains("ctx-0.2.2-aarch64-apple-darwin.tar.gz"));
}
